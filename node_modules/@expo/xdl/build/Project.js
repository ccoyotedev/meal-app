...
function prepareHooks(hooks, hookType, projectRoot, exp) {
  ...
  if (hooks) {
    var _hooks$hookType, _hooks$hookType2;

    (_hooks$hookType = hooks[hookType]) === null || _hooks$hookType === void 0 ? void 0 : _hooks$hookType.forEach(hook => {
      let {
        file
      } = hook;
    ...
    if (hooks[hookType] !== undefined && validHooks.length !== ((_hooks$hookType2 = hooks[hookType]) === null || _hooks$hookType2 === void 0 ? void 0 : _hooks$hookType2.length)) {
...
async function exportForAppHosting(projectRoot, publicUrl, assetUrl, outputDir, options = {}) {
  ...
  delete exp.hooks;
  let validPostExportHooks = prepareHooks(hooks, 'postExport', projectRoot, exp); // Add assetUrl to manifest
...
async function publishAsync(projectRoot, options = {}) {
  ...
  delete exp.hooks;
  let validPostPublishHooks = prepareHooks(hooks, 'postPublish', projectRoot, exp);
  let {
    iosBundle,
    androidBundle
...
async function getSignedManifestStringAsync(manifest, currentSession) {
  var _manifest$owner;
  ...
  const {
    response
  } = await _ApiV().default.clientForUser(currentSession).postAsync('manifest/sign', {
    args: {
